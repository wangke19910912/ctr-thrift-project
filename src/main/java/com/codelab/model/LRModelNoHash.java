...
/**
 * Created by songying on 2017/3/1.
 */
public class LRModelNoHash extends Model{


    private static final Logger LOGGER = LoggerFactory.getLogger(LRModelNoHash.class);
    private Map<String, Float> model = null;
    ...
    protected boolean onModelChanged() {
        ...
        Map<String, Float> modelNew = readModel(conf, modelPath);
    ...
    public Map<String, Float> readModel(Configuration conf, String modelPath) {
        Map<String, Float> model = null;
        try {
            ...
            while ((line = input.readLine()) != null) {
                ...
                if(Math.abs(Float.valueOf(weight)) > minWeight)
                    if (!model.containsKey(fea)) {
                        model.put(fea, Float.valueOf(weight));
                    }
            }
            input.close();
            fin.close();
    ...
    public Double score(Set<String> featureValues, String expId, AdData adData, HistoricalDataCorpus historicalDataCorpus, ContextData contextData, String lookBackWindow, Boolean media) {
        ...
        LOGGER.debug("BIAS_FEATURE {}", common.getData(expId).getKey("BIAS_FEATURE"));
        
        if (model.containsKey(common.getData(expId).getKey("BIAS_FEATURE"))) {
            sum += model.get(common.getData(expId).getKey("BIAS_FEATURE"));
        }
    ...
    public Double score(Map<String, Double> featureMap, String expId, UserData userData, AdData adData, HistoricalDataCorpus historicalDataCorpus, ContextData contextData, String lookBackWindow, Boolean mediaFlag, TypeCollection.Calculation featureType, Boolean optimalFlag) {
        ...
        if (optimalFlag) {
            ...
            for(Map.Entry<String, Double> entry : featureMap.entrySet()){
                fea = entry.getKey();
                if (model.containsKey(fea))
                    sum += model.get(fea) * entry.getValue();
            }
        }
